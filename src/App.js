...
import { predictionItems, testItems, condition, colorSets } from './randomized-parameters';
...
function App() {
 ...
 // const training_ids = Array.from(Array(trainingItems.length).keys());
  ...
  //       counterTotal={counterTotal2} setCounterTotal={setCounterTotal2} condition={condition}
  //       colors={colorSets[1]} 
  //     />
  //   )
  // });
  ...
  var tests = test_ids.map((i) => {
    return (
      <TestPhase key={i} incrementTest={incrementTest}
        test_ids={test_ids} phase={currentPhase} testNumber={testNumber} condition={condition}
        colors={colorSets[0]} 
      />
    )
  });
  ...
  //       test_ids={test2_ids} phase={currentPhase} testNumber={test2Number} condition={condition}
  //       colors={colorSets[1]} 
  //     />
  //   )
  // });
