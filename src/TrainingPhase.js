...
const Introduction = (props)=>{
        ...
        const E_A = causalRule(condition,
            1,0,0,
            1,0,0
            ) ? 'ON' : 'OFF';

        const E_nC = causalRule(condition,
            0, 0, 0,
            0, 0, 1
            ) ? 'ON' : 'OFF';
        ...
        const textSingleNode = <p>If the circle is alone, it is OFF:</p>;

        const textA = <p>When only the {shapeAssignment[0]} is wired to the circle, 
        and the {shapeAssignment[0]} is <b><span style={{color:colors.Aon}}>{colors.Aon}</span></b>, the circle 
        is <span style={{color: E_A == 'ON' ? colors.Eon : 'black' }}><b>{E_A}</b></span>:</p>;

        const textnC = <p>When only the {shapeAssignment[1]} is wired to the circle, 
        and the {shapeAssignment[1]} is <b><span style={{color:colors.Coff}}>{colors.Coff}</span></b>, the 
        circle is <span style={{color: E_nC == 'ON' ? colors.Eon : 'black' }}><b>{E_nC}</b></span>:</p>;
        ...
        const index = Math.min(props.trainingNumber, 1);
        const m = trialContent[display_order[index]].map((i)=>{
        ...
        const m_prime = Array.from(Array(m.length).keys()).map((i)=>{
            let minIndex = Math.min(props.trainingNumber, 1)
            let index = display_order_within[minIndex][i];
            return(m[index])
        })
    ...
    const nextPageText = props.trainingNumber === 0 ? 'Show more examples' : 'Next';
    // the next-page button
    const nextPageButton = <button style={{
        ...buttonStyle, visibility: visNext
    }} onClick={() => handleClick()}>{nextPageText}</button>;

    // the last trial doesn't display any machine, but simply introduces the prediction phase
    const Outro = (props)=>{
    ...
    // controls which jsx content to display
    // (depending on whether this is the last trial)
    const toReturn = props.trainingNumber < 2 ? <span><br></br>
                {newMachineButton}
                ...
                {nextPageButton}
   <br></br></span>
: <Outro incrementTraining={props.incrementTraining} trainingNumber={props.trainingNumber} />
